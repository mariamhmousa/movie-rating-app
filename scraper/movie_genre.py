        ...
        with connection.cursor() as cursor:
            for record in recordList:
                ...
                if media_type == "Movie":
                    ...
                    #genre
                    genre = movie_details.find('li', string="Genre:")
                    if genre is not None:
                        genres = genre.find_next_siblings('li')
                        for g in genres:
                            print(g.get_text().strip())
                            sql = "INSERT IGNORE INTO `movie_genre` (`movie_name`, `release_date`, `genre`) VALUES (%s, %s, %s)"
                            cursor.execute(sql, (movie_name, date_text, g.text))
